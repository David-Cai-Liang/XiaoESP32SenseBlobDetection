...
// Prevent type collision between esp_camera's sensor_t and Adafruit_Sensor's sensor_t
...
#define LED_GPIO_NUM   21 // User LED (Active LOW)
...
#define MAX_W          320
#define MAX_H          240
#define MAX_STACK_SIZE 8192 // Stack entries (32 KB in internal SRAM)
...
// Read IMU sensor data via Adafruit_MPU6050 & build tracking telemetry
TelemetryData buildTelemetry(const Blob &largest) {
  ...
  if (largest.pixels > 0) {
    heartbeat = (uint16_t)(millis() & 0xFFFF);
  ...
  telem.ax = accel.acceleration.x;
  telem.ay = accel.acceleration.y;
  telem.az = accel.acceleration.z;
  telem.tz = gyro.gyro.z;
...
void setup() {
  ...
  digitalWrite(LED_GPIO_NUM, LOW); // LED ON during initialization

  Serial.begin(115200);
  while (!Serial && millis() < 3000);

  // Initialize Wire and MPU6050
  ...
  // Precalculate CIELAB threshold lookup table
  initThresholdLUT();
