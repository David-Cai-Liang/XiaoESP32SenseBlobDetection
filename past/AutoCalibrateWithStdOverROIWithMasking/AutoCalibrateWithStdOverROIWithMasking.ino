...
// Fast lookup tables stored in PSRAM
static LabVal *lab_lut = NULL;
static uint8_t threshold_lut[65536];

// ---------------------------------------------------------
// LAB Conversion & Lookup Table Setup
// ---------------------------------------------------------
...
void initLabLUT() {
  if (psramFound()) {
    lab_lut = (LabVal *)ps_malloc(65536 * sizeof(LabVal));
  } else {
    lab_lut = (LabVal *)malloc(65536 * sizeof(LabVal));
  }

  for (uint32_t px = 0; px < 65536; px++) {
    int L, A, B;
    rgb565ToLab((uint16_t)px, L, A, B);
    lab_lut[px].L = (uint8_t)constrain(L, 0, 255);
    lab_lut[px].A = (int8_t)constrain(A, -128, 127);
    lab_lut[px].B = (int8_t)constrain(B, -128, 127);
  }
}

void updateThresholdLUT(const LabThreshold &t) {
  for (uint32_t px = 0; px < 65536; px++) {
    threshold_lut[px] = (lab_lut[px].L >= t.l_min && lab_lut[px].L <= t.l_max &&
                         lab_lut[px].A >= t.a_min && lab_lut[px].A <= t.a_max &&
                         lab_lut[px].B >= t.b_min && lab_lut[px].B <= t.b_max) ? 1 : 0;
  }
}
...
// Step 1: Calculate LAB statistics from raw ROI pixels
void updateThresholdsFromROI(camera_fb_t *fb) {
  ...
  if (now - last_calc_time >= 150) {
    ...
    for (int y = ROI_Y; y < ROI_Y + ROI_H; y++) {
      ...
      for (int x = ROI_X; x < ROI_X + ROI_W; x++) {
        ...
        LabVal lab = lab_lut[px];
        if (lab.L > 75) continue; // Skip specular glare
        if (lab.L < 25) continue; // Skip deep shadow
        sum_L += lab.L; sum_A += lab.A; sum_B += lab.B;
        count++;
      }
    }
    ...
    if (count > 0) {
      ...
      for (int y = ROI_Y; y < ROI_Y + ROI_H; y++) {
        ...
        for (int x = ROI_X; x < ROI_X + ROI_W; x++) {
          uint16_t px = buf[row_offset + x];
          LabVal lab = lab_lut[px];
          sq_L += (lab.L - mean_L) * (lab.L - mean_L);
          sq_A += (lab.A - mean_A) * (lab.A - mean_A);
          sq_B += (lab.B - mean_B) * (lab.B - mean_B);
        }
      }
      ...
      updateThresholdLUT(current_threshold);
    }
  }
}

// Step 2: Apply color mask to entire frame buffer FIRST
void applyColorMask(camera_fb_t *fb) {
  uint16_t *buf = (uint16_t *)fb->buf;
  int total_pixels = fb->width * fb->height;

  for (int i = 0; i < total_pixels; i++) {
    if (!threshold_lut[buf[i]]) {
      buf[i] = 0x0000; // Zero out non-matching pixels to black
    }
  }
}

// Step 3: Draw Red ROI Square SECOND (so red box remains visible over mask)
void drawROISquare(camera_fb_t *fb) {
  uint16_t *buf = (uint16_t *)fb->buf;
  int stride = fb->width;
  uint16_t RED = 0xF800;
...
void setupCamera() {
  ...
  if (s) {
    ...
    s->set_aec_value(s, 40);    // Manual exposure value
  }
}
...
void loop() {
  ...
  // 1. Compute stats & update threshold
  updateThresholdsFromROI(fb);

  // 2. Apply color mask to whole frame
  applyColorMask(fb);

  // 3. Draw red ROI box over masked frame
  drawROISquare(fb);

  
  // 4. Encode to JPEG and stream over USB CDC
